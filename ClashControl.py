...
if not any(getattr(_cb, "__name__", None) == "_log_slow_gc" for _cb in gc.callbacks):
    ...
    # `from ClashControl import GLOBAL_GUILD_ID, run_nightly_maintenance_routine,
    ...
    # module name "ClashControl" (sys.modules has no entry for "ClashControl", only
...
# ---------------------------------------------------------------------------
#
# 1) NO_DEV_MODE_UPDATES  (this file, ClashControl.py)
...
#
# 3) _DEV_API_THROTTLE_S  (clashcontrol/coc_health.py, line ~49)
...
async def initialize_database() -> None:
    ...
    try:
        ...
        # check_hot_history_schema_parity_sync's docstring, clashcontrol/db_manager.py): a genuine
        ...
        if mismatched_tables:
            logging.critical(
                ...
                "clashcontrol/scripts/repair_history_schema_drift.py / DATABASE_ARCHITECTURE.md."
            )
...
async def _validate_coc_api_keys() -> None:
    ...
    clashcontrol/docs/COPILOT_PITFALLS_COOKBOOK.md Pitfall 24 for the full incident.
...
async def startup_login() -> None:
    ...
    if not CONFIG.no_coc_api:
        ...
        from clashcontrol.coc_health import set_reconnect_callback
...
async def main() -> None:
    ...
    # via clashcontrol/scripts/log_time_gaps.py: a consistent ~4.1-4.2s silent gap
    ...
    # files), not a blocking bug. See clashcontrol/scripts/log_time_gaps.py findings.
    logging.info(
    ...
    from clashcontrol.discord_health import is_discord_available
...
def _archive_move_nightly() -> None:
    ...
    try:
        from clashcontrol.move_old_archive_files import run as _archive_run
    ...
    while not QBcore.shutdown_event.is_set():
        ...
        try:
            ...
            def _post_cycle_cleanup() -> int:
                ...
                from clashcontrol.web_bridge import clear_player_stats_cache
            ...
            if QBcore.memtrace_pending:
                try:
                    ...
                    if _tm.is_tracing():
                        from clashcontrol.QBdiscocmdshelper_admin_command import save_memtrace_snapshot
            ...
            if not QBcore.shutdown_event.is_set():
                ...
                # read against.
                try:
                    from clashcontrol.db_manager import _meminfo_mb as _pc_meminfo
                    _mi = _pc_meminfo()
                    if _mi:
                        logging.info(
...
async def _setup_hook():
    ...
    COMMANDS: list[Any] = [
        ...
        QBdiscordcmds.about,  # type: ignore[misc]
    ...
    # actual channels. CONFIG.tracker_enabled is always `not is_dev_mode` (clashcontrol/config.py).
    ...
    from clashcontrol.QBdiscocmdshelper import remember_app_command_ids
    ...
    from clashcontrol.web_bridge import start_web_bridge
...
async def _run_startup_initialization() -> None:
    ...
    try:
        ...
        try:
            ...
            # refcounting before any collection sees them. See clashcontrol/docs/PERFORMANCE_TUNING.md
        ...
        try:
            from clashcontrol.emojis import ensure_application_emojis
...
async def on_member_join(member: discord.Member) -> None:
    ...
    """
    from clashcontrol.QBdiscocmdshelper import assign_newbie_role
    ...
    try:
        from clashcontrol.ui_tracker import apply_pending_requestor_access
    ...
    try:
        ...
        if cfg_early.get("coc_role_enabled") or cfg_early.get("clan_role_enabled"):
            if str(member.id) in CACHE.user_accounts:
                from clashcontrol.guild_role_manager import sync_roles_for_user
...
async def on_message(message: discord.Message) -> None:
    ...
    from clashcontrol.ui_tracker import handle_tracker_upload_message
...
async def on_raw_reaction_add(payload: discord.RawReactionActionEvent) -> None:
    ...
    """
    from clashcontrol.ui_tracker import handle_tracker_test_reaction
    try:
...
async def _confirm_delete_message_from_channel(channel_id: int, message_id: str, *, log_label: str = "anchored") -> bool:
    ...
    clashcontrol/docs/COPILOT_PITFALLS_COOKBOOK.md Pitfall 15).
    """
    try:
...
async def repost_cwl_management_messages(*, only_if_not_bottom: bool = False, bump_cooldown_seconds: int = PLAYERREGISTRATION_BUMP_COOLDOWN_SECONDS, guild_id: Optional[int] = None) -> None:
    ...
    guild_id: if given, only process this one guild (see repost_anchored_message()'s guild_id
        doc — tracker #0061).
    """
    from clashcontrol.ui_cwl_roster import CwlManagementHubView, add_cwl_management_components
...
async def repost_cwl_player_hub_messages(*, only_if_not_bottom: bool = False, bump_cooldown_seconds: int = PLAYERREGISTRATION_BUMP_COOLDOWN_SECONDS, guild_id: Optional[int] = None) -> None:
    ...
    guild_id: if given, only process this one guild (see repost_anchored_message()'s guild_id
        doc — tracker #0061).
    """
    from clashcontrol.ui_cwl_roster import build_cwl_player_hub_content_and_view
...
async def async_cleanup() -> None:
    ...
    try:
        from clashcontrol.web_bridge import stop_web_bridge
