...
async def _seed_announce_rosters_scenario(db, guild_id: str, *, status: str, assign: bool) -> int:
    ...
    event_id = db.create_cwl_event_sync(guild_id, "2026-08", "discordid1")
    db.set_cwl_event_clans_sync(
        event_id, [{"clan_tag": "#CLAN1", "participating": True, "cwl_start_at": "2026-08-01T08:00Z"}]
    )
    if status != "draft":
    ...
    if assign:
        db.upsert_cwl_assignment_sync(
            event_id, "#P1", "#CLAN1", assignment_source="admin_override", locked=True
        )
    return event_id
...
async def test_announce_rosters_button_disappears_once_everyone_is_announced(db):
    ...
    await db.conn.commit()
    db.upsert_cwl_assignment_sync(
        event_id, "#P2", "#CLAN1", assignment_source="admin_override", locked=True
    )
