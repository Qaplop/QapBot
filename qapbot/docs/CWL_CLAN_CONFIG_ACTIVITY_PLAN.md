...
**OAuth token-endpoint rate limit hit during live-testing (2026-08-16), root-caused and fixed.** Repeatedly opening/closing the Activity in DEV eventually failed with `Setup failed: token exchange failed: 502`. The Worker's `/token` handler already had Discord's real rejection reason in hand (`response.text()` on a non-ok upstream response) but never surfaced it anywhere — neither logged server-side nor returned to the frontend, which only ever read `tokenResponse.status`. Fixed first: the Worker now `console.error`s the real status+body (visible via `wrangler tail`/Cloudflare's dashboard Logs) and includes `detail` in its JSON response; `main.ts` now reads and displays it. That immediately revealed the actual cause via a live `wrangler tail` session during reproduction: `{"error": "invalid_request", "error_description": "You are being rate limited for requesting too many tokens. Please try again later."}` — Discord's own OAuth token-endpoint rate limit, not a bug producing malformed or duplicate requests (the tail log showed exactly one legitimate `authorize()`+`/api/token` pair per genuine launch, no duplicates). Root cause: every single launch — even reopening the identical view a few seconds after closing it — did a completely fresh `authorize()` + token exchange, when `authenticate()` is documented to accept a previously-obtained `access_token` directly (it returns a fresh `expires` regardless of whether the token is new or reused). Fixed: `main.ts` now caches the `access_token` in `sessionStorage` after a successful exchange and, on the next launch, tries `authenticate()` with the cached token FIRST — skipping `authorize()`/`/api/token` entirely — falling back to the full OAuth round-trip only if the cached token is rejected (expired, revoked, or none cached yet). `sessionStorage` rather than a module-level variable, since Discord reloads this script fresh on every launch but keeps the same top-level browsing context alive across repeated launches within one Discord client session.
...
17. ✅ **Clan-config table: horizontal scroll instead of cell wrap, for good this time (2026-08-16).** Item 12's row-height tightening plus the shared-clan-badge inline fix (`.shared-clan-inline`) both fought the same underlying bug piecemeal — any cell whose content wraps to a second line grows that one row taller than its neighbors, which reads as "misaligned" even though the table's columns themselves never actually move. A live-testing report (narrowing the browser window wrapped the Tier text, and separately made the date input + time `<select>` in `.start-time-cell` stack instead of sitting side by side) showed the piecemeal approach doesn't generalize — every new place text could get long enough to wrap is a new instance of the same bug. Fixed at the root instead: `th, td { white-space: nowrap; }` so no cell can ever wrap, and the `<table>` is now wrapped in a `.table-scroll` div (`overflow-x: auto`) so an over-wide table scrolls sideways rather than reflowing — exactly the pattern `enrollmentBoard.ts`'s `.board`/`.column-header-name-text` already used successfully for the same problem. `.start-time-cell`'s flex children got `flex-shrink: 0` (and `flex-wrap: nowrap` stated explicitly, even though it's the default) so the date input and time select always keep their natural side-by-side width instead of shrinking unevenly under space pressure. **General rule for this file going forward: never let a table cell wrap — if content might overflow, make the table scroll, don't let one row grow.**
