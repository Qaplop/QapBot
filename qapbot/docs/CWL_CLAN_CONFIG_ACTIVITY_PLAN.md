...
14. ✅ **New per-guild "Select Timezone" setting, next to "Select Language" (2026-08-10)** — exactly the fallback the project owner originally proposed back when item 9 was first discussed, now actually needed because of item 13's code-block constraint. `guild_config.timezone_offset_minutes INTEGER NOT NULL DEFAULT 0` (idempotent migration, same pattern as `cwl_selected_season`). New `parse_utc_offset()`/`format_utc_offset()` in `QBhelperfunctions.py` (free-text `"+2"`/`"-5"`/`"+5:30"` parsing, not a Select/RadioGroup dropdown — Select caps at 25 options and RadioGroup at 10, neither fits the ~38 distinct real-world UTC offsets including half/quarter-hour ones). New `TimezoneConfigurationModal` in `ui_clan_management.py`, opened via a 5th button ("Select Timezone") added to Basic Config's row 3 (the same row as "Select Language" — Discord's per-row cap of 5, exactly filled). `cwl_start_at_compact()` takes the resolved offset and shifts before formatting; the table's "CWL Start" header gets the offset appended dynamically (`"CWL Start (UTC+5:30)"`, or plain `"CWL Start (UTC)"` at the default `0`).
