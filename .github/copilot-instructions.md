...
- **Format**: `YYYY-MM-DD (N)` heading using TODAY's real date. **`N` is a continuous running counter that does NOT reset per day** — take the highest `N` currently in the file and add 1, whatever date it carries. (Corrected 2026-08-30: this bullet previously said "increments per entry added that day (1, 2, 3...)", which never matched the file — 2026-08-29 ran up to `(31)` and 2026-08-30 continued at `(32)`, not `(1)`. Following the old wording would have produced a duplicate low number.) Never reuse or renumber older entries.
  - **Parallel-session hazard**: this repo is often worked on from several concurrent sessions, and two of them picking "highest + 1" at the same time collide — `(32)` and `(33)` each already appear twice, once dated 2026-08-29 and once 2026-08-30. Re-read the top of `changelog.txt` immediately before writing, and if a duplicate does land, leave it: renumbering rewrites history other entries and commits already reference.
