...
- DO: When a plan-mode session produces an implementation plan for this repo, save the finished plan as a real file in the **project root** (`c:\python\QapBot\<DESCRIPTIVE_NAME>.md`, UPPER_SNAKE_CASE, matching the naming convention of e.g. `CWL_PROD_PERFORMANCE_FIX_PLAN.md`) — never leave it only in the tool-external `~/.claude/plans/` location a coding agent's plan mode may default to. Do this the moment the plan is approved, before starting implementation, not as a later cleanup step.
