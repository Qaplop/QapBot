...
Code: `clashcontrol/db_manager.py` — `initialize()`, `_create_history_schema()`,
...
path, and the shared `attach_history_db()` helper (used by all `clashcontrol/scripts/*.py` maintenance
...
**Repair tool: `clashcontrol/scripts/repair_history_schema_drift.py`** (added 2026-08-14, redesigned
...
`clashcontrol/db_manager.py` (both sync and async), `QBhelperfunctions.py` (`/whois` reliability/
activity/skill), and `clashcontrol/QBdiscocmdshelper_admin_command.py` (`/backfill_cwl_groups`) —
...
also checked for positional (non-named) row access as a separate risk category. Result: `clashcontrol/
scripts/analyze_cwl_rounds.py` (9 query blocks), `clashcontrol/scripts/audit_cwl_league_rank.py` (1 —
confirmed it genuinely reads the drifted `state`/`result` columns, not just theoretically
exposed), and `clashcontrol/scripts/harvest_cwl_war_tags.py` (1) all needed the same fix, now applied
...
it. `clashcontrol/scripts/backfill_group_track_war_updates.py` and `repromote_mid_season_clans.py`
...
at bot startup (`ClashControl.py`'s `initialize_database()`, logged at `CRITICAL` if non-empty).
...
`clashcontrol/scripts/run_history_migration_now.py` and `clashcontrol/scripts/run_db_maintenance_now.py` let
...
- `player_name_search(player_tag PK, name, name_lower)` / `player_name_fts` (FTS5, trigram
  ...
  - **2026-08-17 follow-up fix — `player_name_fts` rowid**: `player_tag` is declared `UNINDEXED`
    ...
    - **v1 → v2**: the first version of this fix (`tag_hash_v1`) derived the rowid from a
      ...
      and PROD's much weaker CPU never finished it at all — it hit `ClashControl.py`'s 30-minute DB-init
...
```python
# ClashControl.py initialization
try:
...
| Skip in polling loop | `ClashControl.py` Phase-1 build | After the `track_war_updates` skip, any clan with `is_deleted=True` is also skipped; no API calls, no errors |
| Auto-restore | `clashcontrol/coc_cache.py` `_update_clan_metadata()` | Any successful `GET /clans/{tag}` response for a clan where `is_deleted=True` → clears to `False` + logs `[CLAN-RESTORED]` + persists |
| DB persistence | `clashcontrol/cache_manager.py` `persist_clan()` | Passes `is_deleted` from cache dict to `save_clan()` |
| DB schema | `clashcontrol/db_manager.py` | `is_deleted BOOLEAN NOT NULL DEFAULT 0`, baked directly into the base `CREATE TABLE IF NOT EXISTS clans` statement (no separate `ALTER TABLE` migration — `db_manager.py` has none currently; see Hot/History DB Split above) |
...
**Note:** Some one-off maintenance scripts under `clashcontrol/scripts/` may import `sqlite3` directly for offline validation/repair.
...
- **`COUNT(*)`/`COUNT(DISTINCT)` over hot+history has no O(1) shortcut**: `wars_count`/
  ...
  `computed_at_utc`), and the plain startup warm-up (`ClashControl._warm_global_db_stats_cache()`,
...
- **2026-07-26 fix**: `_process_league_group_response`'s `clan_name_cache` fallback (used
  ...
  **Bulk repair + permanent self-heal**: the write-time fixes above don't repair rows already
  ...
  re-examined afterwards. Two additions close that: (1) `clashcontrol/scripts/audit_cwl_league_rank.py
...
- Also removed `backfill_cwl_round_numbers()` from db_manager (was a post-season safety net now
  ...
  Also removed the Phase-3 call site in `ClashControl.py` and a stale comment in `cache_manager.py`.
...
- Reported symptom: `/leaderboard` (scope="all", the new default from the entry above) took
  *minutes* to respond on prod, not the sub-second DB query expected from an indexed lookup.
  ...
  Files: clashcontrol/db_manager.py (composite index ×3 creation sites + major_indexes),
...
- Root cause: `idx_wa_player_tag_date` (added above) was created via `await self._conn.execute(...)`
  ...
  `ClashControl.py`'s `on_ready()` wraps in `asyncio.wait_for(..., timeout=60.0)`. Building this index
...
- Fix:
  ...
  Files: clashcontrol/db_manager.py.
...
- Fix: removed the background task entirely (and `_composite_index_build_task`). `initialize()`
  ...
  action or `clashcontrol/scripts/run_db_maintenance_now.py` — both call the same
...
- Fix: added `ClashControl.initialize_database()`, called as a new "Step 1.5" in `on_ready()` — strictly
...
- **General lesson (kept from the previous entry, still the core takeaway)**: a background task
  ...
  Files: ClashControl.py, clashcontrol/db_manager.py.
  Tests: 1467 passed. pyright: 0 errors (ClashControl.py, clashcontrol/db_manager.py).
...
- **Incident**: The 2026-08-01 03:00 UTC nightly window ran `monthly_history_migration()` for the
  ...
  `ClashControl.is_monthly_migration_due()` would have skipped retrying for the rest of August even
...
- **Fix 2**: the final `set_bot_metadata("last_history_migration", ...)` call is now gated on
  ...
  that retry.
- **Recovery tooling**: re-added `clashcontrol/scripts/run_history_migration_now.py` (a prior version of
  ...
  `ClashControl.run_nightly_maintenance_routine()` calls both in sequence, gated by `day == 1`. Restored
...
- **Recovery procedure for an interrupted migration** (this incident or any future one): 1) stop
  ...
  `python clashcontrol/scripts/run_history_migration_now.py --yes` to resume/finish the migration
  (naturally idempotent — safe to re-run), 4) `python clashcontrol/scripts/run_db_maintenance_now.py --yes`
  to checkpoint + VACUUM/REINDEX + ANALYZE and reclaim the space freed by the migration's DELETEs,
  5) restart the bot.
- Files: `clashcontrol/db_manager.py` (`_migrate_table_batch_by_date`, `monthly_history_migration`),
  `clashcontrol/scripts/run_history_migration_now.py` (new), `clashcontrol/docs/CLAN_AND_WAR_CYCLE_ARCHITECTURE.md`
...
- **Automatic-path safety**: without a bounded automatic path, simply restarting the bot mid-backlog
  would immediately re-trigger an *unbounded* `monthly_history_migration()` via `ClashControl.py`'s
  ...
  default 90) is now passed to both automatic call sites in `ClashControl.py`
...
- Files: `clashcontrol/db_manager.py`, `clashcontrol/scripts/run_history_migration_now.py`, `clashcontrol/config.py`,
  `ClashControl.py`, `README.md`, `tests/unit/test_history_migration_time_budget.py` (new, 3 tests).
  1470 tests pass.
...
- Files: `ClashControl.py` (`is_monthly_migration_due`), `QBdiscordcmds.py` (admin call site),
...
- Files: `ClashControl.py` (`periodic_main()`'s migration-due call sites). 1474 tests pass (no new tests —
...
- Files: `ClashControl.py` (`periodic_main()`), `clashcontrol/config.py` (new
...
- Files: `ClashControl.py` (`run_nightly_maintenance_routine`), `QBdiscordcmds.py` (admin call site),
  `clashcontrol/config.py` (new `history_migration_admin_budget_minutes` + env var), `README.md`.
  1474 tests pass.
...
- Files: `clashcontrol/db_manager.py` (`fast_bulk_history_migration`, `_bulk_move_chunk`,
  `_drop_war_attacks_secondary_indexes`, `_rebuild_war_attacks_secondary_indexes`),
  `clashcontrol/scripts/run_history_migration_now.py` (`--fast`/`--fast-chunk-size`),
...
- **Operational workaround** (for anyone hitting `SQLITE_FULL` on a manual `CREATE INDEX`/`VACUUM`
  ...
  default.
- Files: `clashcontrol/db_manager.py` (`fast_bulk_history_migration`).
...
- **Root cause**: `periodic_main()`'s scheduled trigger (`_maint_due` in `ClashControl.py`) gated firing on
...
- Files: `ClashControl.py` (`periodic_main()`'s `_maint_due` block). 1519 tests pass.
...
- Files: `clashcontrol/db_manager.py` (schema, backfill, 2 writers extended, 2 new readers:
  `search_player_names_sync`/`search_player_tags_by_prefix_sync`), `clashcontrol/cache_manager.py`
  (tuple shape, `set_player_name()`, rollout-flag delegation), `clashcontrol/coc_cache.py` (writer
  switched to `set_player_name()`), `clashcontrol/web_bridge.py` (guest-search tag-prefix mode
  delegation), `QBdiscordcmds.py` (`/whois` threading fix), `clashcontrol/config.py`
...
- **Stopgap applied (this entry)**: new `CONFIG.history_migration_enabled` (env
  ...
  note above. `clashcontrol/scripts/run_history_migration_now.py` bypasses the flag by design and remains
...
- Files: `clashcontrol/config.py` (`history_migration_enabled` + env var), `ClashControl.py`
...
- Files: `clashcontrol/config.py`, `QBcore.py`, `clashcontrol/ui_common.py`, `clashcontrol/db_manager.py`,
  `ClashControl.py`, `QBdiscordcmds.py`, `clashcontrol/scripts/run_history_migration_now.py`, `README.md`.
...
- **Guarded**: `tests/unit/test_history_retention_cutoff.py` now asserts both properties
  ...
  and passes at 75.
- Files: `clashcontrol/config.py`, `README.md`, `tests/unit/test_history_retention_cutoff.py`.
...
switch. Removed from `config.py`, `ClashControl.is_history_migration_due()`, `README.md` and the
tests.
...
- Files: `clashcontrol/config.py`, `ClashControl.py`, `QBdiscordcmds.py`, `clashcontrol/coc_health.py`,
  `clashcontrol/scripts/run_history_migration_now.py`, `README.md`,
...
- Files: `clashcontrol/config.py`, `clashcontrol/db_manager.py`, `README.md`, `.env.example`,
...
**Code:**
- Database manager: `clashcontrol/db_manager.py`
- Cache manager: `clashcontrol/cache_manager.py`
- Consistency check: `/admin integrity` command (replaces deprecated check_database_consistency.py)
- On-demand nightly maintenance runner (still present): `clashcontrol/scripts/run_db_maintenance_now.py`
...
**Documentation:**
- Architecture: `clashcontrol/docs/CODE_STRUCTURE.md`
