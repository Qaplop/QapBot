...
from clashcontrol.ui_common import claim_action, lock_buttons, release_action
...
# Derived from CWL_LEAGUE_ORDER (clashcontrol/constants.py — the codebase's actual single source of
...
def _make_cwl_settings_toggle_signup_mode_callback(view: discord.ui.View):
    ...
    async def callback(interaction: discord.Interaction) -> None:
        ...
        from clashcontrol.web_bridge import bump_enrollment_version
        await bump_enrollment_version(interaction.guild.id)
        await _refresh_parent(view, interaction, "cwl_settings")

        if now_extended:
            from clashcontrol.emojis import bench_emoji
            from clashcontrol.i18n import t
...
def _make_cwl_settings_toggle_callback(view: discord.ui.View):
    async def callback(interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            try:
                from ClashControl import repost_cwl_management_messages
...
def _make_cwl_settings_toggle_player_hub_callback(view: discord.ui.View):
    ...
    async def callback(interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            try:
                from ClashControl import repost_cwl_player_hub_messages
...
class CwlCoordinatorConfigurationView(discord.ui.View):
    ...
    async def _on_save(self, interaction: discord.Interaction) -> None:
        ...
        async with self._lock:
            ...
            try:
                from clashcontrol.guild_role_manager import sync_cwl_coordinator_role
    ...
    async def _on_notify(self, interaction: discord.Interaction) -> None:
        ...
        """
        from clashcontrol.i18n import t
        from clashcontrol.QBdiscocmdshelper_cwl import _dm_guard_blocks  # pyright: ignore[reportPrivateUsage]  # deliberately shared, see docstring above
...
def build_cwl_signup_response_view(
    ...
    and click doesn't silently break it (registered once via add_dynamic_items(), ClashControl.py).
...
async def _apply_cwl_signup_response(
    ...
    from clashcontrol.QBdiscocmdshelper_cwl import propagate_cwl_player_response
...
def build_cwl_reminder_response_view(
    ...
    add_dynamic_items() in ClashControl.py."""
    view = discord.ui.View(timeout=None)
...
async def _check_cwl_admin_or_leader_permission(interaction: discord.Interaction) -> bool:
    ...
    from clashcontrol.config import CONFIG
    from clashcontrol.QBdiscocmdshelper import check_admin_or_leader_permission
...
async def refresh_cwl_management_hub_message(guild_id: int, mode: Optional[str] = None) -> None:
    ...
    from clashcontrol.QBdiscocmdshelper_cwl import format_clan_management_cwl_settings, format_clan_management_cwl_management
...
class CwlManagementHubView(discord.ui.View):
    ...
    the periodic maintenance repost (`only_if_not_bottom=True`, ClashControl.py's `main()`) skips a
    ...
    ClashControl.py's on_ready() Step 8b calls `repost_cwl_management_messages()` with no
...
# Phase 5b, pulled forward into Phase 3: repost_cwl_player_hub_messages() (ClashControl.py) has a
...
async def build_cwl_player_hub_content_and_view(channel: Any, guild_id_int: int) -> Tuple[str, discord.ui.View, Optional[discord.Embed]]:
    ...
    counterpart to repost_cwl_management_messages()'s own inline builder (ClashControl.py). Static
