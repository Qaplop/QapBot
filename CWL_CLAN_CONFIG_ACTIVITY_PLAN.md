...
3. **A real platform limit, not a bug**: checked whether a successful save could auto-close the Activity — the Embedded App SDK's entire command list (checked directly against the installed package) has no `close`/`exit`/`minimize` command at all. An Activity can never programmatically close itself; only the user can, via Discord's own collapse control. Since that specific ask isn't achievable here, a successful Save now shows an explicit "✓ Saved — you can close this window now" state instead.
