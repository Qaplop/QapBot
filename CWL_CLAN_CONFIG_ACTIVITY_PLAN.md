...
### Phase B — Bridge API + tunnel ✅ code shipped 2026-08-09 (DEV environment; tunnel setup + live verification still pending)
`qapbot/web_bridge.py`: an `aiohttp.web` app with `GET`/`POST /api/cwl/clan-config` (plus `/api/health`), started from `_setup_hook()` and stopped from `async_cleanup()` — no-ops entirely unless both `WEB_BRIDGE_PORT`/`WEB_BRIDGE_SECRET` are set (new `BotConfig` fields, same "0/empty = disabled" convention as the rest of the config). Bound to `127.0.0.1` only, per the plan's security section — `cloudflared`, not this process, is what makes it externally reachable. 7 tests in `tests/discord/test_web_bridge.py` (secret gate, admin re-verification including the configured-super-admin bypass, GET payload shape, POST persistence + Hub-message refresh trigger) — 1618 total tests pass.
...
**Still pending** (needs the project owner's local machine, not something committed to the repo): installing/running `cloudflared` alongside the DEV bot process, setting the tunnel URL + a shared secret as `BRIDGE_URL`/`BRIDGE_SECRET` Worker secrets (`wrangler secret put ... --env dev`) and as `WEB_BRIDGE_PORT`/`WEB_BRIDGE_SECRET` in `.env`, restarting the DEV bot, and redeploying the Worker — only after all of that is a live end-to-end round trip (Worker → tunnel → bridge → real CWL data) actually verifiable, the way Phase A's OAuth round-trip was verified live in Discord before being marked done. The frontend already has its half of this wired (fetches `/api/cwl/clan-config` with the verified access token and renders the raw JSON response) as the "hardcoded test payload" smoke test the original phase description called for — it just hasn't been exercised against a live tunnel yet.
