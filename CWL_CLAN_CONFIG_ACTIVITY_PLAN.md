...
3. ✅ **Season-selection dropdown.** `GET /api/cwl/clan-config` now accepts an optional `?season=` query param and always returns `available_seasons` (every season with a saved event, plus the resolved "current" season and whichever season was requested, newest-first) alongside `has_own_data`. `activity/client/src/clanConfigTable.ts` renders this as a `<select>` in the header; changing it calls `main.ts`'s `onSeasonChange`, which does a full re-fetch-and-rerender for that season (simpler and safer than patching table state in place — this is an interactively-edited screen, not a tight loop). `POST /api/cwl/clan-config` now accepts an optional `season` in the body (defaulting to `resolve_current_cwl_season()` if omitted, preserving old behavior) so Save always targets whichever season is currently selected, not always the guild's "current" one.
4. ✅ **Carry-over prompt.** The bridge additionally reports `carry_over_available`/`carry_over_season`/`previous_clans` — true only when the selected season has no own data yet *and* a previous season has participating clans to offer. Critically, `clans` itself always stays at the plain Phase E.2 defaults; carry-over is never auto-applied. The frontend shows an explicit "No settings saved yet for this season. Carry over configuration from {season}? [Yes] [No]" banner in that case; "Yes" copies `previous_clans`' `roster_size`/`cwl_start_at` onto the working table (and sets `participating=true`) and re-renders the rows in place, "No" just dismisses the banner — neither reaches the server until the user clicks Save.
5. ✅ **Timezone-aware date/time.** Supersedes Phase C's "always UTC by convention, never a timezone conversion" note above, which described the Phase C baseline, not the final design. `clanConfigTable.ts` now converts the bridge's UTC `"YYYY-MM-DDTHH:MMZ"` to/from the browser's local timezone at exactly two functions (`utcStringToLocalParts`/`localPartsToUtcString`) — everywhere else in the file works purely in local terms, so the "invisible to the user" requirement is satisfied by construction rather than by discipline. The header shows the resolved IANA timezone name (`Intl.DateTimeFormat().resolvedOptions().timeZone`) so the user knows what "local" means without guessing. The "Start Time (UTC)" table header became "Start Time (local)".

Bridge-side: 5 new tests in `tests/discord/test_web_bridge.py` covering explicit season selection, the `available_seasons` list, carry-over offered/not-offered, and POST honoring an explicit season — 1621 total tests pass. Frontend-side: `npm run typecheck` and `npm run build` both clean; deployed to `cwl-clan-config-dev.pages.dev` and confirmed live via the deployed bundle containing the new `season-select`/`carry-over-banner` markers — **manual live click-through in the DEV Discord guild still pending** (season switching, the carry-over banner, and that displayed/saved times survive a round-trip through a non-UTC local timezone correctly, are things only a real browser+Discord session can confirm).
