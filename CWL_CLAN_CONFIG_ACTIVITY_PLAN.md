...
**Three refinements from that live test, shipped same day**:
1. Start-time picker restricted to `:00`/`:15`/`:30`/`:45` via `datetime-local`'s native `step="900"` attribute (seconds) — no custom picker needed.
...
3. ~~"A real platform limit, not a bug": no close/exit API exists~~ — **corrected same day**: this was wrong, based on only grepping the SDK's `commands/` folder. `discordSdk.close(RPCCloseCodes.CLOSE_NORMAL, '')` is a real, exported top-level method (a raw `CLOSE` protocol frame, not a request/response "command," which is why it wasn't where I first looked — confirmed by finding it directly in the installed package's own `.d.ts` files after being asked to double-check). Save now shows "✓ Saved — closing…" for ~1.2s (with an "Edit again" button that cancels the pending close) then actually closes the Activity.
